...
function Graph(name, element) {
  ...
  // TODO bisectDate, formatValue, etc, http://bl.ocks.org/mbostock/3902569
  // TODO look like http://code.shutterstock.com/rickshaw/examples/lines.html

  // RULERS on axes
  var xruler = this.xruler = this.svg.select('.x.axis').append('g').attr('class', 'x ruler');
  ...
  var yruler = this.yruler = this.svg.select('.y.axis').append('g').attr('class', 'y ruler');
  ...
  var sidebar = d3.select(this.element).append('div').attr('class', 'sidebar');
  var title = sidebar.append('label').attr('class', 'title');
...
Graph.prototype = {
  ...
  rescale: function(data) {
    ...
    this.x.domain([
      d3.min(data, function(c) { return d3.min(c.values, gettime); }),
      d3.max(data, function(c) { return d3.max(c.values, gettime); })
    ]);

    this.y.domain([
      0,
      d3.max(data, function(c) { return d3.max(c.values, getvalue); })
    ]).nice();
  ...
  render: function() {
    ...
    var curves = this.svg.selectAll('.curve').data(curvedata, getid);
    ...
    this.svg.selectAll('.curve').select('path')
      .attr('d', function(d) { return self.ignored.has(d.id) ? '' : self.line(d.values); });
    ...
    var events = this.svg.selectAll('.event-slot').data(eventdata, getid);
    events.enter().append('g').attr('class', 'event-slot');
    events.exit().remove();

    var lines = this.svg.selectAll('.event-slot')
      .style('stroke', function(d) { return self.ignored.has(d.id) ? 'none' : self.color(d.id); })
    .selectAll('.event')
      .data(function (e) { return e.values; }, function (d) { return d.time.getTime(); });
    lines.enter().append('line').attr('class', 'event').attr('y2', height);
    lines.exit().remove();

    this.svg.selectAll('.event')
      .attr('transform', function (d) { return 'translate(' + self.x(d.time) + ',0)'; });

    // TODO select curves and events, intersect with curves and show values/hovers

    let data = curvedata.concat(eventdata);

    var legends = this.legend.selectAll('label').data(data, getid);

    var newlegend = legends.enter().append('label');
    newlegend.append('input').attr('type', 'checkbox').attr('checked', 'true').on('click', function(d) {
      if (self.ignored.has(d.id)) {
    ...
    newlegend.append('span').attr('class', 'color').style('background-color', function(d) { return self.color(d.id); });
