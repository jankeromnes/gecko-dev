...
class SingleTestRunner(BaseTestRunner):
  ...
  def UpdateFilter(self, failed_tests):
    ...
    if self._running_on_emulator:
      filter_file.write('# Additional list of suppressions from emulator\n')
    else:
  ...
  def SetUp(self):
    """Sets up necessary test environment for the test suite."""
  ...
  def TearDown(self):
    """Cleans up the test environment for the test suite."""
